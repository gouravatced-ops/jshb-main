...
class ProcessDocumentGenerationQueue extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($pendingJobs as $job) {
            ...
            try {
                ...
                $app = Application::with('allottee')->find($job->application_id);
                $recipientEmail = null;
                $ccEmail = 'system@adms.jshb.computered.co.in';
                
                if ($app && $app->allottee && $app->allottee->email) {
                    $recipientEmail = $app->allottee->email;
                    $applicantName = $app->allottee->applicant_name ?? 'Applicant';
                    $subject = "Document Generated - Application {$app->application_no}";
                    $body = "Dear {$applicantName},<br><br>Your document for Application No: <strong>{$app->application_no}</strong> has been successfully generated by the system (Batch ID: {$batchProgram->id}).<br><br>Please log in to your dashboard to view and download your generated document.<br><br>Regards,<br>JSHB System";
                    
                    \Illuminate\Support\Facades\Mail::to($recipientEmail)
                        ->cc($ccEmail)
                        ->queue(new \App\Mail\GenericNotificationMail($subject, $body));
                        
                    $log->info("Email queued for Allottee ID: {$app->allottee->id} and CC'd to system");
                }
    ...
    private function generateDocument($job, $log)
    {
        ...
        if ($isPossession) {
            ...
            if ($currentStep) {
                AllotteeProcessStep::completeStep($allottee->id, 'doucment-process', $currentStep->sub_menu_key, $user_id);
                AllotteeProcessStep::unlockNextStep($allottee->id, $currentStep->step_no);
            }
        } elseif ($isAgreement) {
            AllotteeProcessStep::completeStep($allottee->id, 'doucment-process', 'agreement-document-letter', $user_id);
            $currentStep = AllotteeProcessStep::where([
                'allottee_id' => $allottee->id,
                'menu_key' => 'doucment-process',
            ...
            AllotteeProcessStep::completeStep($allottee->id, 'doucment-process', 'generate-allotment', $user_id);
