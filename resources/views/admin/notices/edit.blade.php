...
@section('title', 'Edit Notice | JSHB')
    ...
    <div class="form-wrapper">
        <div class="form-main">
            ...
            <div class="form-header">
                <div>
                    <h4>Edit Notice & Announcement</h4>
                    <p>Update notice details and targeting settings.</p>
                </div>
            ...
            <form action="{{ route('admin.notices.update', $notice) }}" method="POST">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group" style="flex: 1;">
                        ...
                        <select name="notice_type" class="form-select" required>
                            ...
                            <option value="warning" {{ old('notice_type', $notice->notice_type) == 'warning' ? 'selected' : '' }}>Warning</option>
                            <option value="new" {{ old('notice_type', $notice->notice_type) == 'new' ? 'selected' : '' }}>New Feature / Update</option>
                            <option value="info" {{ old('notice_type', $notice->notice_type) == 'info' ? 'selected' : '' }}>General Information</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="message" id="summernote" class="form-control" required>{{ old('message', $notice->message) }}</textarea>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select name="target_user_type" id="target_user_type" class="form-select">
                            ...
                            <option value="{{ $type }}" {{ old('target_user_type', $notice->target_user_type) == $type ? 'selected' : '' }}>{{ ucfirst($type) }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group" id="division_wrapper">
                        ...
                        <select name="target_division_id" id="target_division_id" class="form-select">
                            ...
                            <option value="{{ $div->id }}" {{ old('target_division_id', $notice->target_division_id) == $div->id ? 'selected' : '' }}>{{ $div->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group" id="user_wrapper">
                        ...
                        <select name="target_user_id[]" id="target_user_id" class="form-select" multiple="multiple" style="height: 100px;">
                            @php $selectedUsers = json_decode($notice->target_user_id, true) ?? []; @endphp
                            @foreach($users as $u)
                            <option value="{{ $u->id }}" data-type="{{ $u->user_type }}" data-div="{{ $u->division_id }}" {{ in_array($u->id, old('target_user_id', $selectedUsers)) ? 'selected' : '' }}>{{ $u->name }} ({{ $u->username ?? $u->email }})</option>
                            @endforeach
                        </select>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="datetime-local" name="start_date" class="form-control" value="{{ old('start_date', $notice->start_date ? $notice->start_date->format('Y-m-d\TH:i') : now()->format('Y-m-d\TH:i')) }}" min="{{ ($notice->start_date && $notice->start_date < now()) ? $notice->start_date->format('Y-m-d\TH:i') : now()->format('Y-m-d\TH:i') }}">
                    </div>
                    
                    <div class="form-group">
                        <label>End Date</label>
                        <input type="datetime-local" name="end_date" class="form-control" value="{{ old('end_date', $notice->end_date ? $notice->end_date->format('Y-m-d\TH:i') : '') }}" min="{{ now()->format('Y-m-d\TH:i') }}">
                    </div>
                </div>
                ...
                <div class="form-row" style="display: flex; gap: 20px; align-items: center; flex-wrap: wrap;">
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <input type="checkbox" name="notice_in_software" id="notice_in_software" value="1" {{ old('notice_in_software', $notice->notice_in_software) ? 'checked' : '' }}>
                        <label for="notice_in_software" style="margin: 0;">Notice in Software (Default)</label>
                    </div>
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <input type="checkbox" name="send_email" id="send_email" value="1" {{ old('send_email', $notice->send_email) ? 'checked' : '' }}>
                        <label for="send_email" style="margin: 0;">Email</label>
                    </div>
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <input type="checkbox" name="send_sms" id="send_sms" value="1" {{ old('send_sms', $notice->send_sms) ? 'checked' : '' }}>
                        <label for="send_sms" style="margin: 0;">SMS</label>
                    </div>
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <input type="checkbox" name="send_whatsapp" id="send_whatsapp" value="1" {{ old('send_whatsapp', $notice->send_whatsapp) ? 'checked' : '' }}>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const oldUserIds = @json(old('target_user_id', json_decode($notice->target_user_id, true) ?? []));
