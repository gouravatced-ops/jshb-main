...
<div class="card border-0 shadow-sm" style="padding:0;">
    <div class="d-flex align-items-center justify-content-between px-4 py-3 border-bottom bg-white">
        <div>
            ...
            <div class="text-muted small">{{ trim(($allottee->allottee_name ?? '') . ' ' . ($allottee->allottee_middle_name ?? '') . ' ' . ($allottee->allottee_surname ?? '')) ?: 'Allottee' }}</div>
        </div>
        <a class="btn btn-outline-secondary" href="{{ route('admin.allottees.index') }}">Back to List</a>
    </div>

    <div style="display:grid;grid-template-columns:280px 1fr;min-height:640px;">
...
<style>
    .allottee-sidebar { border-right: 1px solid #e5e7eb; background: linear-gradient(180deg, #f8fafc 0%, #eff6ff 100%); }
    .allottee-tab { display:block; padding:11px 14px; margin:8px; border-radius:10px; color:#1e293b; text-decoration:none; font-weight:600; border:1px solid transparent; }
    .allottee-tab:hover { background:#e2e8f0; border-color:#cbd5e1; }
    .allottee-tab.active { background:#0f766e; color:#fff; box-shadow:0 8px 18px rgba(15,118,110,.25); }
    .allottee-tab.locked { opacity:.55; cursor:not-allowed; pointer-events:none; }
    .badge-step{display:inline-flex;align-items:center;justify-content:center;width:24px;height:24px;border-radius:50%;font-size:11px;margin-right:8px;font-weight:700}
    .badge-step.completed{background:#16a34a;color:#fff}
    .badge-step.pending{background:#f59e0b;color:#fff}
    .badge-step.locked{background:#94a3b8;color:#fff}
</style>
