...
<div class="card">
    ...
    <form method="GET"
        action="{{ route('admin.allottees.index') }}"
        id="allotteeFilterForm">
            ...
            <tbody>
                ...
                <tr>
                    <td colspan="7" style="text-align:center;padding:32px 20px;color:var(--text-light);">
                        No allottees found for selected filters.
                    </td>
                </tr>
                @endforelse
            </tbody>
    ...
    <div class="table-pagination">
        <span>
            Showing <strong>{{ $allottees->firstItem() }}</strong> to <strong>{{ $allottees->lastItem() }}</strong> of <strong>{{ $allottees->total() }}</strong> allottees
        </span>
        <div class="pagination-btns">
            ...
            @else
            <a class="pag-btn" href="{{ $allottees->previousPageUrl() }}"><i class="fa-solid fa-chevron-left"></i></a>
            @endif

            @foreach($allottees->getUrlRange(1, $allottees->lastPage()) as $page => $url)
            <a class="pag-btn {{ $page === $allottees->currentPage() ? 'active' : '' }}" href="{{ $url }}">{{ $page }}</a>
            @endforeach

            @if($allottees->hasMorePages())
            <a class="pag-btn" href="{{ $allottees->nextPageUrl() }}"><i class="fa-solid fa-chevron-right"></i></a>
            @else
