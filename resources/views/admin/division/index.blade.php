...
@section('title', 'Division List | JESA')
...
<div class="card">
    ...
    <div class="card-head">
        ...
        <div class="card-actions">
            <form method="GET" action="{{ route('admin.divisions.index') }}" class="search-box" onsubmit="return false;">
                <i class="fa-solid fa-magnifying-glass"></i>
                <input type="text" id="divisionSearchInput" name="search" value="{{ $search }}" placeholder="Search divisions..." autocomplete="off">
            </form>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-btns">
                                <a class="action-btn edit" href="{{ route('admin.divisions.edit', $division) }}" title="Edit">
    ...
    @if($divisions->total() > 0)
        <div class="table-pagination" id="divisionPagination">
            <span>
                Showing <strong>{{ $divisions->firstItem() }}</strong> to <strong>{{ $divisions->lastItem() }}</strong> of <strong>{{ $divisions->total() }}</strong> divisions
            </span>
            <div class="pagination-btns">
                ...
                @else
                    <a class="pag-btn" href="{{ $divisions->previousPageUrl() }}"><i class="fa-solid fa-chevron-left"></i></a>
                @endif

                @foreach($divisions->getUrlRange(1, $divisions->lastPage()) as $page => $url)
                    <a class="pag-btn {{ $page === $divisions->currentPage() ? 'active' : '' }}" href="{{ $url }}">{{ $page }}</a>
                @endforeach

                @if($divisions->hasMorePages())
                    <a class="pag-btn" href="{{ $divisions->nextPageUrl() }}"><i class="fa-solid fa-chevron-right"></i></a>
                @else
